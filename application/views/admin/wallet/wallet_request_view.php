...
<div class="page-wrapper">
	<div class="content container-fluid">
		...
		<div class="page-header">
			<div class="row">
				<div class="col">
					<h3 class="page-title"><?php echo(!empty($wallet['lg_admin_wallet']))?($wallet['lg_admin_wallet']) : 'Wallet';  ?></h3>
				</div>
						...
						<div class="table-responsive">
							<table class="table table-hover table-center mb-0 service_table" >
                                ...
                                <tbody>
                                    ...
                                    foreach ($list as $rows) {
										if(!empty($rows['date'])){
											...
											$date_change = date(settingValue('date_format'), strtotime($datef[0]));
											$date = $date_change.' '.$time;
										}else{
											$date='-';
										}
										...
										if($rows['role'] == 1) {
			                    			...
			                    			$user_name = $this->db->where('id',$rows['user_provider_id'])->get('providers')->row_array();
			                  			}
			                  ...
			                  if($rows['role'] == 2) {
			                    ...
			                    $this->db->where('lang_type', $user_lang);
			                    $lang_pro = $this->db->get('users_lang')->row_array();
			                    $user_name = $this->db->where('id',$rows['user_provider_id'])->get('users')->row_array();
			                  }
			                  $user_pro_name = (!empty($lang_pro['name']))?($lang_pro['name']) : $user_name['name'];
							  	$currency_code_old = $rows['currency_code'];
								$currentWallet = get_gigs_currency($rows['current_wallet'], $currency_code_old, settings('currency'));
								$debitWallet = get_gigs_currency($rows['debit_wallet'], $currency_code_old, settings('currency'));
									echo '<tr>
										<td>'.$i++.'</td>
										<td>'.$date.'</td>
										<td>
											<h2 class="table-avatar">
												...
												<a href="javascript:void(0);">'.str_replace('-', ' ', $user_pro_name).'</a>
											</h2>
										</td>
										<td>'.$rows['user_mobile'].'</td>
										<td><span class="badge badge-'.$color.'">'.ucfirst($role).'</span></td>
										<td>'.$det.'</td>
										<td>'.currency_code_sign(settings('currency')).$currentWallet.'</td>
										<td>'.currency_code_sign(settings('currency')).$debitWallet.'</td>
										<td>'.$rows['reason'].'</td>
										<td><span class="badge badge-'.$color_t.'">'.$message_t.'</span></td>';
										echo '<td><select class="form-control form-control-sm request_status_change" name="ser_status" data-id="'.$rows['id'].'" data-providerid="'.$rows['user_provider_id'].'" data-amount="'.$rows['debit_wallet'].'">
			...
			<div class="card-body">
				<form id="upload_document" action="<?php echo base_url();?>admin/wallet/wallet_request_status" method="post" autocomplete="off" enctype="multipart/form-data">
					...
					<div class="form-group">
						...
						<input class="form-control" type="file"  accept="image/*,.pdf,.html,.doc,.docx" name="upload_doc" id="upload_doc">
					</div>
					<div class="mt-4">
