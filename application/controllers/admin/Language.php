...
class Language extends CI_Controller {
    ...
    public function AppPageList($lang) 
    {
        ...
        $this->data['lang'] = $lang;
        $this->data['langName'] = $this->db->select('')->get_where('language', array('language_value'=>$lang))->row()->language;   
    ...
    public function pages_language_new($page,$lang)
    {        
        ...
        $this->data['lang_keywords'] =  $this->db->query('SELECT lke.sno,lke.lang_key,lke.lang_value as def_lang_value,
        (SELECT lsub.lang_value FROM app_language_management lsub WHERE lsub.lang_key=lke.lang_key AND lsub.language="'.$lang.'" AND lsub.page_key="'.$page.'" LIMIT 1) as lang_value 
        FROM `app_language_management` lke WHERE lke.language="en" AND lke.page_key=? AND lke.lang_value!="";',array($page))->result();
        $this->data['langName'] = $this->db->select('')->get_where('language', array('language_value'=>$lang))->row()->language;
        // $this->data['module'] = $this->db->select('')->get_where('language_module', array('id'=>$id))->row()->module_name;
        // echo $this->db->last_query();die;

        
         if($this->data['langName']){
            $this->data['page'] = 'pages_language_new';
            $this->load->vars($this->data);
            $this->load->view($this->data['theme'] . '/template');
        }else{
            $message = (!empty($this->data['language_content']['lg_admin_page_found'])) ? $this->data['language_content']['lg_admin_page_found'] : 'Page Not Found. ';
            $this->session->set_flashdata('error_message', $message);
            redirect(base_url() . "languages-settings");
        }
    }
    public function update_module_app_language_keyword() {
        $data = $this->input->post();
        if($this->input->post())
        {
            $where=array('language'=>$data['lang'],'lang_key'=>$data['lang_key']);
            if($this->db->select('')->get_where('app_language_management', $where)->row()){
                $this->db->where($where);
                $array = array(
                    'page_key' => $data['page_key'],
                    'lang_value' => $data['lang_value'],
                );    
                if($this->db->update('app_language_management', $array)) {
                    echo '1';
                } else {
                    echo '2';
                }
                ...
                if($this->db->insert('app_language_management', $array)) {
                    echo '1';
                } else {
                    echo '2';
                }
    ...
    public function InsertAppKeyword() {
        ...
        if (!empty($insertPage) && $insertPage != '') {
            ...
            $message = (!empty($this->data['language_content']['lg_admin_not_add_page'])) ? $this->data['language_content']['lg_admin_not_add_page'] : 'Pages added not successfully. ';
            $this->session->set_flashdata('error_message', $message);
    ...
    public function delete_appkeyword() {
        ...
        $table_data['status'] = 0;    
            $this->db->where('p_id', $id);
            if ($this->db->update('pages', $table_data)) {
                $message = (!empty($this->data['language_content']['lg_admin_appkeyword_delete'])) ? $this->data['language_content']['lg_admin_appkeyword_delete'] : 'AppKeyword deleted successfully successfully. ';
                $this->session->set_flashdata('success_message', $message);
                echo 1;
            } else {
                $message = (!empty($this->data['language_content']['lg_something_went_wrong'])) ? $this->data['language_content']['lg_something_went_wrong'] : 'Something went wrong, Try again';
    ...
    public function addLanguage() {        
        if($this->input->post('form_submit') == true) {
            ...
            if ($this->db->insert('language', $lang_data)) {
                ...
                $message = (!empty($this->data['language_content']['lg_admin_language_add'])) ? $this->data['language_content']['lg_admin_language_add'] : 'Language added successfully. ';
                $this->session->set_flashdata('success_message', $message);
                redirect(base_url() . "languages-new");
            } else {
                $message = (!empty($this->data['language_content']['lg_something_went_wrong'])) ? $this->data['language_content']['lg_something_went_wrong'] : 'Something went wrong, Try again';
                $this->session->set_flashdata('error_message', $message);
                redirect(base_url() . "languages-new");
    ...
    public function addLanguageList() {        
        $language = $this->db->get_where('language_default', array('id'=>$_POST['language_id']))->row_array();
        $lang_data['language'] = $language['language'];
        $lang_data['language_value'] = $language['language_value'];
        $lang_data['status'] = $language['status'];
        $lang_data['tag'] = 'ltr';
        $checkLang = $this->db->select('')->get_where('language', array('language_value'=>$language['language_value']))->row()->language;
        if($checkLang){
            $message = (!empty($this->data['language_content']['lg_admin_language_exist'])) ? $this->data['language_content']['lg_admin_language_exist'] : 'Language already added. ';
            $this->session->set_flashdata('error_message', $message);
            redirect(base_url() . "languages-settings");
        }
        else if ($this->db->insert('language', $lang_data)) {
            // language_file_create();
            $message = (!empty($this->data['language_content']['lg_admin_language_add'])) ? $this->data['language_content']['lg_admin_language_add'] : 'Language added successfully. ';
            $this->session->set_flashdata('success_message',  $message);
            redirect(base_url() . "languages-settings");
