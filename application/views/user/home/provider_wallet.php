                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="wallet-details">
                                    ...
                                    <h3><?php echo currency_conversion($user_currency_code) . number_format($service_amount1,2); ?></h3>
								...
								<form action="<?= base_url() ?>user/wallet/payment" method="post" id="paypal_payment">
									<div class="form-group">
                                        ...
                                        <?php } if(settingValue('paysolution_show') == 1) { ?>
                                                <div class="col-4">
                                                    <span>
                                                        <input id="gigs_payment_radio7" type="radio" name="group2" id="paysolution"  value="paysolution">
                                                        <label for="gigs_payment_radio7"><img src="<?php echo base_url() . "assets/img/paysolution.png"; ?>"></label>
                                                    </span>
                                                </div>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <?php if (!empty($wallet_history)) {
                             ...
                             ?>
                                <table id="order-summary" class="table table-center mb-0" autocomplete = "off">
                                <?php } else { ?>
                                    ...
                                    <tbody>
                                        ...
                                        if (!empty($wallet_history)) {
                                            ...
                                            foreach ($wallet_history as $key => $value) {
                                                ...
                                                if(file_exists($value['transaction_document'])) {
                                                 
                                                    $files = base_url() .  $value['transaction_document'];
                                                } else {
                                                ...
                                                if(file_exists($value['transaction_document'])) {
                                                    if($extension == 'pdf') { 
                                                        $document = '<a class="text-info" href="'.$files.'" download><span><i class="fa fa-file-pdf" aria-hidden="true"> Download PDF</i></span></a>';
                                                         } else if($extension == 'xlsx') {
                                                            $document = '<a class="text-info" href="'.$files.'" download><span><i class="fa fa-file-excel" aria-hidden="true"> Download Excel</i></span></a>'; 
                                                         } else {
                                                            
                                                             $document = '<a class="text-info" href="'.$files.'" download><span><i class="fa fa-download" aria-hidden="true"></i> Download File</i></span></a>';
                                                        }
                                                } else {
                                                    $document = '-';
                                                }
                                                ...
                                                if($value['payment_type']== 1) {
                                                    $payment_type = (!empty($user_language[$user_selected]['lg_cod'])) ? $user_language[$user_selected]['lg_cod'] : $default_language['en']['lg_cod'];
                                                } else if($value['payment_type']==2) {
                                                    $payment_type = (!empty($user_language[$user_selected]['lg_wallet'])) ? $user_language[$user_selected]['lg_wallet'] : $default_language['en']['lg_wallet'];    
                                                } else {
                                                   $payment_type = '-'; 
                                                }
                                                    $service_amount3 = $value["debit_wallet"];
									...
									<td>' . ($key + 1) . '</td>
									<td>' . $wall_time . '</td>
									<td>' . currency_conversion($user_currency_code) . '' . number_format($service_amount1,2) . '</td>
									<td>' . currency_conversion($user_currency_code) . '' . number_format($service_amount3,2) . '</td>
                                    <td>' . currency_conversion($user_currency_code) . '' . number_format($service_amount2,2) . '</td>
									<td>' . currency_conversion($user_currency_code) . '' . number_format($service_amount5,2) . '</td>
						...
						<div class="razorpay_details">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="mode" id="payment_mode">
                                    <option value=""><?php echo (!empty($user_language[$user_selected]['lg_Select_Payment_Mode'])) ? $user_language[$user_selected]['lg_Select_Payment_Mode'] : $default_language['en']['lg_Select_Payment_Mode']; ?></option>
