...
extract($_POST);
  ...
  public function delete_chat(){
    extract($_POST);
    if($self_token != '' && $partner_token != '') {
      $ret=$this->Chat_model->delete_chat($self_token,$partner_token);
      if($ret){
        $this->session->set_flashdata('success_message',$this->web_validation_array['lg_chats_delete_successfully']);
        echo 1;
        }else{
          echo 2;
        }
