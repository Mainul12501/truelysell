...
$sand_query = $this->db->query(" SELECT * FROM `payment_gateways` WHERE `id` = 1 ");
$sanbox = $sand_query->row_array();

$live_query = $this->db->query(" SELECT * FROM `payment_gateways` WHERE `id` = 2 ");
$live = $live_query->row_array();
			...
			<div class="modal-body">
				...
				<div class="text-center">
					<?php if($this->session->userdata('type')=="user"){?>
						<button type="button" class="can_btn btn btn-theme py-2 px-4 text-white mx-auto" id="cancel_booking" ><?php echo (!empty($user_language[$user_selected]['lg_Submit'])) ? $user_language[$user_selected]['lg_Submit'] : $default_language['en']['lg_Submit']; ?></button>
					<?php }else{?>
						<button type="button" class="can_btn btn btn-theme py-2 px-4 text-white mx-auto" id="provider_cancel_booking"><?php echo (!empty($user_language[$user_selected]['lg_Submit'])) ? $user_language[$user_selected]['lg_Submit'] : $default_language['en']['lg_Submit']; ?></button>
					<?php }?>
				</div>
			</div>
		</div>
...
<div class="modal fade" id="reschedule">
	<div class="modal-dialog modal-dialog-centered">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title"><?php echo (!empty($user_language[$user_selected]['lg_reschedule'])) ? $user_language[$user_selected]['lg_reschedule'] : $default_language['en']['lg_reschedule']; ?></h5>
			...
			<div class="modal-body">
				...
				<div class="form-group">
                    ...
                    <input class="form-control" type="text" name="reschedule_booking_date" id="reschedule_booking_date" />

                    <input type="hidden" name="provider_id" id="cancel_provider_id" value="<?php echo $service_details['user_id'] ?>">
                    <input type="hidden" name="service_id" id="cancel_service_id" value="<?php echo $service_details['id'] ?>">
					<p class="error_cancel error" ><?php echo (!empty($user_language[$user_selected]['lg_Reason_required'])) ? $user_language[$user_selected]['lg_Reason_required'] : $default_language['en']['lg_Reason_required']; ?></p>
				</div>
				<div class="form-group">
				...
				<div class="text-center">
					<?php if($this->session->userdata('type')=="user"){?>
						<button type="button" class="reschedule_btn btn btn-theme py-2 px-4 text-white mx-auto" id="user_reschedule_booking" ><?php echo (!empty($user_language[$user_selected]['lg_Submit'])) ? $user_language[$user_selected]['lg_Submit'] : $default_language['en']['lg_Submit']; ?></button>
					<?php }else{?>
						<button type="button" class=" reschedule_btn btn btn-theme py-2 px-4 text-white mx-auto" id="user_reschedule_booking"><?php echo (!empty($user_language[$user_selected]['lg_Submit'])) ? $user_language[$user_selected]['lg_Submit'] : $default_language['en']['lg_Submit']; ?></button>
					<?php }?>
				</div>
...
<!-- Account Alert Modal -->
<div class="modal" id="account_alert_provider_appointment">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h4 class="modal-title"><?php echo (!empty($user_language[$user_selected]['lg_Alert'])) ? $user_language[$user_selected]['lg_Alert'] : $default_language['en']['lg_Alert']; ?></h4>
			...
			<div class="modal-body">
				<div class="form-group">
					<p><?php echo (!empty($user_language[$user_selected]['lg_Please_Fill_Appointment_Settings'])) ? $user_language[$user_selected]['lg_Please_Fill_Appointment_Settings'] : $default_language['en']['lg_Please_Fill_Appointment_Settings']; ?></p>
					<button type="button" class="btn btn-primary get_pro_appointment"><?php echo (!empty($user_language[$user_selected]['lg_Ok'])) ? $user_language[$user_selected]['lg_Ok'] : $default_language['en']['lg_Ok']; ?></button>
				</div>
			</div>
				...
				<div class="account-content">
					<div class="account-box">
						...
						<form action="" method='post' id="new_third_page">
							...
							<div class="form-group">
								<button id="registration_submit" type="submit" class="btn login-btn user-register"><?php echo (!empty($user_language[$user_selected]['lg_Register'])) ? $user_language[$user_selected]['lg_Register'] : $default_language['en']['lg_Register']; ?></button>
							</div>
					...
					<div class="account-box">
						<div class="login-right">
							...
							<form method='post' id="new_third_page_user">
								...
								<div class="form-group">
									<button id="registration_submit_user" type="submit" class="login-btn btn user-register"><?php echo (!empty($user_language[$user_selected]['lg_Register'])) ? $user_language[$user_selected]['lg_Register'] : $default_language['en']['lg_Register']; ?></button>
								</div>
								<div class="account-footer text-center">
									<?php echo (!empty($user_language[$user_selected]['lg_already_have_account'])) ? $user_language[$user_selected]['lg_already_have_account'] : $default_language['en']['lg_already_have_account']; ?> <a href="javascript:void(0);" data-dismiss="modal" data-toggle="modal" data-target="#tab_login_modal"><?php echo (!empty($user_language[$user_selected]['lg_login'])) ? $user_language[$user_selected]['lg_login'] : $default_language['en']['lg_login']; ?></a>
								</div>
...
<?php 
	...
	if($cat_view == 'Popular category') {
		...
		$query = $this->db->query("SELECT GROUP_CONCAT(DISTINCT(category)) as id, categories_lang.category_name, COUNT(*) count FROM services LEFT JOIN categories_lang on categories_lang.id = services.category GROUP BY category HAVING count > 0 ORDER BY count DESC LIMIT 4");
    ...
    $linkQu = $this->db->get()->row();
	$link_wid_language=$linkQu?$linkQu->title:($quick_link?(str_replace('_', ' ', $quick_link->page_title)):'Quick Link');
	...
	<div class="footer-top">
		<div class="container">
			<div class="row">
				<?php 
				if($categories->widget_showhide == 1 && !empty($result)) { 
				?>
							...
							<div class="footer-widget footer-menu">
								<?php 
			                        foreach ($cat_wid_language as $cat_wid) { ?>
			    						<h2 class="footer-title">
											<?php echo(!empty($cat_wid['title']))?($cat_wid['title']) : 'Category Widget';  ?>
										</h2>
		                        ...
		                        <div class="about">
		                            <div class="contactdet">
		                                <ul>
										<?php foreach ($category_result as $res) { 
											...
											<?php echo $cat_name->category_name;?></a></li>
								...
								<div class="about">
		                            <div class="contactdet">
										...
										<?php
											...
											$quickLink = json_decode($quick_link->link,true);
											foreach ($quickLink[$lang_val] as $key => $link) { 
												if(!empty($link)) {
												?>
							...
							<div class="footer-widget footer-menu">
								...
								<div class="about">
		                            <div class="contactdet">
		                                <ul>
											<?php
											if($this->db->query(" SELECT * FROM `page_content` WHERE `status` =  1 and page_slug='about-us'")->row()){
											?>
											<li><a href="<?php echo base_url();?>about-us">
													<?php 
                                                    echo (!empty($user_language[$user_selected]['lg_about'])) ? $user_language[$user_selected]['lg_about'] : $default_language['en']['lg_about']; 
                                                    ?>
												</a>
											</li>
											<?php } 
											if($this->db->query(" SELECT * FROM `page_content` WHERE `status` =  1 and page_slug='faq'")->row()){
											?>
											<li>
												<a href="<?php echo base_url();?>faq">
													<?php 
														echo (!empty($user_language[$user_selected]['lg_FAQ'])) ? $user_language[$user_selected]['lg_FAQ'] : $default_language['en']['lg_FAQ']; 
													?>
												</a>
											</li>
											<?php } ?>
						...
						<div class="copyright">
							<div class="row">
										...
										<ul class="policy-menu">
											<?php $crLinks = json_decode($copyright->link);
												foreach($crLinks as $key => $crlink) { 
													...
													<li>
													<a href="<?php echo $crlink->url; ?>">
														<?php echo  $crlink->name; ?>
														<?php 
															?>
													</a>
												</li>
								...
								<?php } else {?>
										...
										<ul class="policy-menu">
											<?php 
											if($this->db->query(" SELECT * FROM `page_content` WHERE `status` =  1 and page_slug='privacy-policy'")->row()){
											?>
											<li>
												<a href="<?php echo base_url();?>privacy">
												<?php 
												echo (!empty($user_language[$user_selected]['lg_Privacy_Policy'])) ? $user_language[$user_selected]['lg_Privacy_Policy'] : $default_language['en']['lg_Privacy_Policy']; 
												?>
												</a>
											</li>
											<?php } 
											if($this->db->query(" SELECT * FROM `page_content` WHERE `status` =  1 and page_slug='terms-conditions'")->row()){
											?>
											<li><a href="<?php echo base_url();?>terms-conditions">
											<?php 
											echo (!empty($user_language[$user_selected]['lg_Terms'])) ? $user_language[$user_selected]['lg_Terms'] : $default_language['en']['lg_Terms']; 
											?></a></li>
											<?php } ?>
										</ul>
									</div>
					...
					<div class="modal-body">
						...
						<div id="login_form_div">
										...
										<div class="form-group">
											<div class="row">
												<div class="col-12">
													...
													<span class="error" id="mailid_error"></span>
												</div>
											</div>
										</div>
							<div class="form-group">
							...
							<div class="form-group">
								...
								<span class="error" for='otp_number' id='otp_error_msg_login'></span>
							</div>
						
							<br>
							<p class="user_forgot_pwd"><?php echo (!empty($user_language[$user_selected]['lg_forgot_password'])) ? $user_language[$user_selected]['lg_forgot_password'] : $default_language['en']['lg_forgot_password']; ?>  <a href="javascript:void(0);" data-dismiss="modal" data-toggle="modal" data-target="#modal-forgot"><?php echo (!empty($user_language[$user_selected]['lg_click_get_link'])) ? $user_language[$user_selected]['lg_click_get_link'] : $default_language['en']['lg_click_get_link']; ?></a></p>
							<span id="err_respwd"></span>
							<div>
								<button id='emailregistration_finals' type="button" class="login-btn" ><?php echo (!empty($user_language[$user_selected]['lg_login'])) ? $user_language[$user_selected]['lg_login'] : $default_language['en']['lg_login']; ?></button>
							</div><br>	
										<div class="account-footer text-center">
										...
										<div class="form-group">
											...
											<div class="row">
												<div class="col-12">
													<input type="hidden" name="login_mode" id="login_mode" value="1">
													<input type="hidden" name="csrf_token_name" value="<?php echo $this->security->get_csrf_hash(); ?>" id="login_csrf">
													<input  type="hidden" value="" id="login_countrycode" />
...
<?php } ?>
<div class="modal account-modal fade" id="modal-forgot" data-keyboard="false" data-backdrop="static">
	<div class="modal-dialog modal-dialog-centered">
		<div class="modal-content">
			<div class="modal-header p-0 border-0">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
				...
				<div id="login_form_divs">
					<div class="account-content">
						<div class="account-box">
								...
								<div class="form-group">
									<div class="row">
										<div class="col-12">
											...
											<input type="hidden" name="csrf_token_name" value="<?php echo $this->security->get_csrf_hash(); ?>" id="login_csrf">
											<label><?php echo (!empty($user_language[$user_selected]['lg_Email'])) ? $user_language[$user_selected]['lg_Email'] : $default_language['en']['lg_Email']; ?></label>
											<input class="form-control forgot_email" type="text" name="login_email" id="forgot_email" placeholder="<?php echo (!empty($user_language[$user_selected]['lg_enter_email'])) ? $user_language[$user_selected]['lg_enter_email'] : $default_language['en']['lg_enter_email']; ?>">
											<span id="forgotmailid_error" class="error"></span>
										</div>
									</div>
								</div>
							<div class="form-group">
							...
							<span id="err_forgotrespwd"></span>
							<div>
								<button id='user_forgot_pwd' type="button" class="login-btn" ><?php echo (!empty($user_language[$user_selected]['lg_Submit'])) ? $user_language[$user_selected]['lg_Submit'] : $default_language['en']['lg_Submit']; ?></button>
							</div><br>
							</div>
						</div>
					</div>
		...
		<div class="modal fade" id="deleteProviderAccount" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title"><?php echo (!empty($user_language[$user_selected]['lg_delete_account'])) ? $user_language[$user_selected]['lg_delete_account'] : $default_language['en']['lg_delete_account']; ?></h5>
						<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
					</div>
					<div class="modal-body">
						<p id="msg"><?php echo (!empty($user_language[$user_selected]['lg_want_to_delete_account'])) ? $user_language[$user_selected]['lg_want_to_delete_account'] : $default_language['en']['lg_want_to_delete_account']; ?> </p>
						<p id="user_id"></p>
						<p id="user_type"></p>
					</div>
					<div class="modal-footer">
						<a href="javascript:;" class="btn btn-success delete_confirm"><?php echo (!empty($user_language[$user_selected]['lg_Yes'])) ? $user_language[$user_selected]['lg_Yes'] : $default_language['en']['lg_Yes']; ?></a>
						<button type="button" class="btn btn-danger delete_cancel" data-dismiss="modal"><?php echo (!empty($user_language[$user_selected]['lg_Cancel'])) ? $user_language[$user_selected]['lg_Cancel'] : $default_language['en']['lg_Cancel']; ?></button>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="modal fade" id="alertModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		    <div class="modal-dialog modal-dialog-centered">
		        <div class="modal-content">
		            <div class="modal-header">
		                <h5 class="modal-title" id="acc_title"><?php echo (!empty($user_language[$user_selected]['lg_delete_alert_account'])) ? $user_language[$user_selected]['lg_delete_alert_account'] : $default_language['en']['lg_delete_alert_account']; ?></h5>
		                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
		                    <span aria-hidden="true">&times;</span>
		                </button>
		            </div>
		            <div class="modal-body">
		                <p id="acc_msg"><?php echo (!empty($user_language[$user_selected]['lg_book_service_not_completed'])) ? $user_language[$user_selected]['lg_book_service_not_completed'] : $default_language['en']['lg_book_service_not_completed']; ?></p>
		            </div>
		        </div>
		    </div>
		</div>
		...
		<div class="modal fade" id="deleteUserAccount" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title" id="acc_title"><?php echo (!empty($user_language[$user_selected]['lg_delete_account'])) ? $user_language[$user_selected]['lg_delete_account'] : $default_language['en']['lg_delete_account']; ?></h5>
						<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
					</div>
					<div class="modal-body">
						<p id="msg"><?php echo (!empty($user_language[$user_selected]['lg_want_to_delete_account'])) ? $user_language[$user_selected]['lg_want_to_delete_account'] : $default_language['en']['lg_want_to_delete_account']; ?> </p>
		...
		if (strcmp($page, 'index') !== 0 && strcmp($module, 'about') !== 0 && strcmp($module, 'contact') !== 0 && strcmp($module, 'privacy') !== 0 && strcmp($page, 'user_dashboard') !== 0 && strcmp($page, 'user_reviews') !== 0 && strcmp($page, 'user_payment') !== 0 && strcmp($page, 'user_change_password') !== 0 && strcmp($page, 'provider_dashboard') !== 0  && strcmp($page, 'my_service_new') !== 0 && strcmp($page, 'my_service_inactive') !== 0 && strcmp($page, 'provider_bookings') !== 0   && strcmp($page, 'payout_settings') !== 0 && strcmp($page, 'appointment-settings') !== 0 && strcmp($page, 'provider_availability') !== 0 && strcmp($page, 'provider_reviews') !== 0  && strcmp($page, 'provider_change_password') !== 0) {
			?>
		
		<script src="<?php echo base_url();?>assets/plugins/datatables/datatables.min.js"></script>

		<?php if (strcmp($page, 'provider_wallet') !== 0 && strcmp($page, 'provider_payment') !== 0 && strcmp($page, 'user_bookings') !== 0 ) { ?>
		...
		<input type="hidden" name="location_type" id="location_type" value="<?=settingValue('location_type')?>">
		<?php if(settingValue('location_type') == 'live') { ?>
...
<script src="<?php echo $base_url; ?>assets/js/chat.js"></script>
		...
		<?php 
			...
			if(strcmp($page, 'index') !== 0 && strcmp($module, 'about') !== 0 && strcmp($module, 'contact') !== 0 && strcmp($module, 'privacy') !== 0 && strcmp($page, 'user_dashboard') !== 0 && strcmp($page, 'user_reviews') !== 0 && strcmp($page, 'user_payment') !== 0 && strcmp($page, 'user_change_password') !== 0 && strcmp($page, 'provider_dashboard') !== 0  && strcmp($page, 'my_service_new') !== 0 && strcmp($page, 'my_service_inactive') !== 0 && strcmp($page, 'provider_bookings') !== 0 && strcmp($page, 'provider_settings') !== 0  && strcmp($page, 'payout_settings') !== 0 && strcmp($page, 'appointment-settings') !== 0 && strcmp($page, 'provider_availability') !== 0 && strcmp($page, 'provider_reviews') !== 0 && strcmp($page, 'provider_payment') !== 0 && strcmp($page, 'provider_change_password') !== 0) {
			
			if(!empty($sanbox['api_key']) || !empty($live['api_key'] && $this->uri->segment(1)=="user-wallet" || $this->uri->segment(1)=="provider-subscription" || $this->uri->segment(1)=="provider-wallet")){ ?>
