		...
		<div class="page-header">
			<div class="row">
				...
				<div class="col-auto text-end">
					...
					<a class="btn btn-primary" onclick="export_language();">
					...
					<a class="btn btn-primary" href="<?php echo base_url() . 'language-module' ?>">
						<i class="fas fa-suitcase mr-2"></i>Modules</i>
					</a>
				</div>
			</div>
		</div>
									...
									foreach ($language as $lang) {
										...
										<tr>
											...
											<td>
												<div>
													<div class="status-toggle">
														<input id="tag_<?php echo $lang->id; ?>" class="check language_tag"
															data-id="<?php echo $lang->id; ?>" type="checkbox" <?php if ($lang->tag == 'rtl') {
																   echo 'checked';
											...
											<td>
												<div>
													<div class="status-toggle">
														<input id="default_<?php echo $lang->id; ?>"
															class="check default_lang" data-id="<?php echo $lang->id; ?>"
															data-status="<?php echo $lang->default_language; ?>"
															type="checkbox" <?php if ($lang->default_language == 1) {
																echo 'checked';
															} ?> 	<?php if ($this->session->userdata('role') != 1) {
																	  echo 'disabled';
																  } ?>>
														<label for="default_<?php echo $lang->id; ?>"
															class="checktoggle">checkbox</label>
													</div>
												</div>
											</td>
												...
												<div>
													<div class="status-toggle" disabled>
														<input <?php echo $attr; ?> id="status_<?php echo $lang->id; ?>"
															class="check language_status" data-id="<?php echo $lang->id; ?>"
															type="checkbox" <?php if ($lang->status == 1) {
																echo 'checked';
															} ?> 	<?php if ($this->session->userdata('role') != 1) {
																	  echo 'disabled';
																  } ?>>
														<label for="status_<?php echo $lang->id; ?>" class="checktoggle"
															disabled>checkbox</label>
													</div>
												</div>
											</td>
											<td class="text-end">
												<a class="btn btn-sm bg-info-light mr-2"
													href="<?php echo base_url() . 'lang-module-list?type=web&language=' . $lang->language_value; ?>"
													title="Web Translation">
													<i class="fas fa-language mr-1"></i>
													<?php echo (!empty($admin_settings['lg_admin_web'])) ? ($admin_settings['lg_admin_web']) : 'Web'; ?>
												</a>
												<a class="btn btn-sm bg-warning-light mr-2"
													href="<?php echo base_url() . 'app-page-list/' . $lang->language_value; ?>"
													title="App Translation">
													<i class="fas fa-language mr-1"></i>
													<?php echo (!empty($admin_settings['lg_admin_app'])) ? ($admin_settings['lg_admin_app']) : 'App'; ?>
												</a>
												<a class="btn btn-sm bg-primary-light mr-2"
													href="<?php echo base_url() . 'lang-module-list?type=admin&language=' . $lang->language_value; ?>"
													title="App Translation">
													<i class="fa fa-language mr-1"></i>
													<?php echo (!empty($admin_settings['lg_admin_admin'])) ? ($admin_settings['lg_admin_admin']) : 'Admin'; ?>
												</a>
												<a href="<?php echo base_url() . 'edit-languages/' . $lang->language_value; ?>"
													class="btn btn-sm bg-success-light mr-2" title="Edit">
													<i class="far fa-edit"></i>
												</a>
												<?php if ($lang->language_value != 'en') { ?>
													<a href="#" class="btn btn-sm bg-danger-light mr-2 delete_language"
														data-id="<?php echo $lang->language_value; ?>">
														<i class="far fa-trash-alt"></i>
													</a>
												<?php } ?>
											</td>
											<td>
												<a class="btn btn-primary btn-sm lang_code" href="javascript:void(0);"
													data-toggle="modal" data-target="#importmodal"
													data-lang="<?php echo $lang->language_value; ?>">
													<i class="fas fa-cloud-upload-alt mr-2"></i>
													<?php echo (!empty($admin_settings['lg_admin_web'])) ? ($admin_settings['lg_admin_web']) : 'Web'; ?>
												</a>
												<a class="btn btn-primary btn-sm lang_app_code" href="javascript:void(0);"
													data-toggle="modal" data-target="#importappmodal"
													data-lang="<?php echo $lang->language_value; ?>">
													<i class="fas fa-cloud-upload-alt mr-2"></i>
													<?php echo (!empty($admin_settings['lg_admin_app'])) ? ($admin_settings['lg_admin_app']) : 'App'; ?>
												</a>
...
<div class="modal" id="importappmodal" tabindex="-1" role="dialog">
	<div class="modal-dialog modal-dialog-centered" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5>
					<?php echo (!empty($admin_settings['lg_amdin_app_language_upload'])) ? ($admin_settings['lg_amdin_app_language_upload']) : 'App Language File Upload'; ?>
				</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			...
			<div class="modal-body">
				<form action="<?php echo base_url() . 'admin/language_new/importapplang' ?>" method="post"
					enctype="multipart/form-data">
					<input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>"
						value="<?php echo $this->security->get_csrf_hash(); ?>" />
...
<div class="modal fade" id="exportModal" tabindex="-1" role="dialog" aria-labelledby="exportModalLabel"
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exportModalLabel">Export Language</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
				...
				<div class="form-group">
					<?php foreach ($language as $lang) { ?>
						<a class="btn btn-primary"
							href="<?php echo base_url() . 'admin/Language_new/exportlang/' . $lang->language_value ?>">
