<?php
	...
	class Booking_model extends CI_Model {
		...
		{
			return $this->db->get_where('book_service',array('id'=>$id))->row_array();
		}
		...
		public function booking_success($inputs)
     	{
			...
			if($inputs['additional_services']){
				$stripe['additional_services'] = $inputs['additional_services'];
			}
			...
			$stripe['tax_details']  = $inputs['tax_details'];
			if(isset($inputs['status'])) {
				$stripe['status'] = $inputs['status'];
			}
