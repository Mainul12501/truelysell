							...
							<ul>
								...
								<li class="nav-item">
									<a href="<?php echo base_url()?>payout-settings" class="nav-link <?= ($this->uri->segment(1)=="payout-settings")?'active':'';?>" >
										...
										<span>											
											<?php echo (!empty($user_language[$user_selected]['lg_payout_settings'])) ? $user_language[$user_selected]['lg_payout_settings'] : ($default_language['en']['lg_payout_settings']??"Payout Settings"); ?>
										</span>
									</a>
								</li>
								...
								<li class="nav-item">
									<a href="<?php echo base_url()?>appointment-settings" class="nav-link <?= ($this->uri->segment(1)=="appointment-settings")?'active':'';?>" >
								...
								<!-- <li class="nav-item">
									<a href="<?php echo base_url()?>provider-profile-verification"  class="nav-link <?= ($this->uri->segment(1)=="provider-profile-verification")?'active':'';?>" >
										<i class="fas fa-hashtag"></i>
										<span>Profile Verification</span>
										 <span><?php echo (!empty($user_language[$user_selected]['lg_profile_verification'])) ? $user_language[$user_selected]['lg_profile_verification'] : $default_language['en']['lg_profile_verification']; ?></span> 
									</a>
								</li> -->
						...
						foreach ($result as $res) {
							...
							<li class="nav-item">
								<?php if($booking_count == 0) { ?>
								 	<a href="javascript:void(0);" id="delete_account" class="nav-link delete_account" data-id="<?php echo $this->session->userdata('id') ?>" data-type="<?php echo $this->session->userdata('usertype') ?>"><i class="fas fa-trash"></i>Delete Account</a>
								 <?php } else { ?>
								 	<a href="javascript:void(0);" class="nav-link" data-toggle="modal" data-target="#alertModal"><i class="fas fa-trash"></i> <?php echo (!empty($user_language[$user_selected]['lg_delete_account'])) ? $user_language[$user_selected]['lg_delete_account'] : $default_language['en']['lg_delete_account']; ?></a>
								 <?php } ?>
							</li>
							</ul>
                        </div>
