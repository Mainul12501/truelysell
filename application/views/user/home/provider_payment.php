									...
									if(count($services)>0){
										...
										foreach($services as $row){ 
									 	...
									 	if(!empty($row['reject_paid_token'])){
											...
											$user_name = $this->db->where('id',$row['user_id'])->get('users')->row_array();
											...
											<tr>
												<td>
													...
													<a href="javascript:void(0);">
														<img src="<?php echo base_url().$service_images;?>" class="pro-avatar" alt=""> <?=$service_name['service_name']??$row['service_title'];?>
													</a>
												</td>
												<td>
													<img class="avatar-xs rounded-circle" src="<?php echo $image;?>" alt=""> <?= !empty($users_name) ? $users_name : '-'; ?>
												</td>
												<?php 
